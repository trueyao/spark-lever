...
private[spark] class TaskSetManager(
  ...
  //query method is added by Liuzhiyi
  ...
  {   //不是说要匹配给定的locality吗？为啥每次都先返回PROCESS_LOCAL的task ?这就是为了满足本地性，优先调度满足本地性的task
    ...
    if (TaskLocality.isAllowed(maxLocality, TaskLocality.NO_PREF)) {
      ...
      for (index <- dequeueTaskFromList(execId, pendingTasksWithNoPrefs)) {
        return Some((index, TaskLocality.PROCESS_LOCAL, false)) //这里居然是PROCESS_LOCAL ？
      }
    }
    ...
    : Option[TaskDescription] =
  {
    if (!isZombie) {
      ...
      if (maxLocality != TaskLocality.NO_PREF) {
        allowedLocality = getAllowedLocalityLevel(curTime) //这个函数有什么意思呢？看下面的问题。已解决：allowLocality可能比maxLocality小
        if (allowedLocality > maxLocality) { //意思是如果allowedLocality=NODE_LOCAL,而maxLocality=PROCESS_LOCAL，条件将会成立，为啥？
  ...
  def recomputeLocality() {
    ...
    currentLocalityIndex = getLocalityIndex(previousLocalityLevel)  //还用之前的localityLevel。比如之前是NODE_LOCAL，现在新添加了executor，
  }                                                                 //recomputeLocality之后还是用NODE_LOCAL
