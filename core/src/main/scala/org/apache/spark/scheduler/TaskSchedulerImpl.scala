  ...
  def resourceOffers(offers: Seq[WorkerOffer]): Seq[Seq[TaskDescription]] = synchronized {
    ...
    for (o <- offers) {
      ...
      if (!executorsByHost.contains(o.host)) {
        ...
        executorAdded(o.executorId, o.host) //只是从failedepoch（之前failed的node）中去掉。这里为啥不添加到executorsByHost ?
        newExecAvail = true                 //好像是lazy的做法，在resourceOfferSingleTaskSet中有添加
      }
    ...
    val tasks = shuffledOffers.map(o => new ArrayBuffer[TaskDescription](o.cores))  //默认是一个task占一个core
    ...
    for (taskSet <- sortedTaskSets; maxLocality <- taskSet.myLocalityLevels) {
      ...
      } while (launchedTask)  //为啥lauchedTask为true，要一直做上面的事情呢？表明这个taskSet成功发起，继续下一个taskSet
    }

    if (tasks.size > 0) {
      hasLaunchedTask = true
    }
    return tasks //这个tasks通过前面的各种操作和函数调用，并没有发生改变吧？因为都是传值传参啊
  }
