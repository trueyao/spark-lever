...
import org.apache.spark.streaming.scheduler.{StreamInputInfo, ReceivedBlockInfo}
  ...
  override def compute(validTime: Time): Option[RDD[T]] = {
    val blockRDD = {
      ...
      if (validTime < graph.startTime) {
        ...
        val inputInfo = StreamInputInfo(id, blockInfos.map(_.numRecords).sum, blockInfos.map(_.totalSize).sum)
        ssc.scheduler.inputInfoTracker.reportInfo(validTime, inputInfo)
        logInfo(s"YY:stream ${id} has ${blockInfos.map(_.numRecords).sum} records.")
